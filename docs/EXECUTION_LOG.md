...
**Pedido:** Investigar e corrigir o lint quebrado em ambos os workspaces (backend e frontend); criar `.github/workflows/ci.yml` rodando em push e pull_request para main, com steps de lint, build e teste (sem deploy); revisar sintaxe da workflow (actionlint ou leitura manual); registrar decisões em `docs/DECISIONS.md`.

**Prompt gerado:** Plano em `C:\Users\User\.claude\plans\crie-github-workflows-ci-yml-rodando-em-composed-waffle.md` — dois subagentes Explore investigaram (1) configuração do workspace e versão do Node, (2) causa raiz do lint quebrado em ambos os workspaces. Confirmadas duas decisões arquiteturais não explícitas via `AskUserQuestion`: versão do Node para o runner de CI (Node 20 LTS, conforme `engines.node: ">=20"` do package.json raiz), ruleset de lint (padrão idiomático de cada framework, sem Prettier).

**Status:** Executado e verificado (subagente `roadmap-verifier` pendente — requisito antes de remover qualquer prompt de CI do roadmap, mas implementação técnica está completa e verde: todos os comandos passam localmente).

**Resultado:** Backend: adicionados `eslint`, `typescript-eslint`, `@eslint/js`, `globals` como devDependencies; criado `backend/eslint.config.mjs` (flat config com Recommended + exceção para DI do NestJS); `npm run lint --workspace=backend` passa sem erros. Frontend: adicionados `eslint`, `eslint-config-next`, `@eslint/eslintrc` como devDependencies; criado `frontend/eslint.config.mjs` (flat config com `@next/eslint-plugin-next`, React, React Hooks, suporte a React 19 com `settings.react.version: detect`); script `lint` em `frontend/package.json` atualizado de `next lint` (descontinuado em Next.js 16) para `eslint .`; `npm run lint --workspace=frontend` passa sem erros. Workflow: criado `.github/workflows/ci.yml` com gatilhos em `push`/`pull_request` para `main`, rodando Node 20 na `ubuntu-latest`, 6 steps (install → lint backend → lint frontend → build backend → build frontend → test). Sintaxe revisada manualmente (actionlint não disponível no ambiente). Documentação: entrada adicionada a `docs/DECISIONS.md` cobrindo (a) ESLint 9 flat config em ambos workspaces, (b) Node 20 LTS no runner de CI, (c) ausência de Prettier/formatador adicional. `docs/MASTER_CHECKLIST.md` atualizado (CI/CD validação marcado concluído). Todos os testes passam (27 testes: 20 Jest + 7 node:test no backend, 8 Vitest no frontend); build passes for both workspaces.
